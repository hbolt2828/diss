...
prop_deaths_NorthWest <- 398 / 1188  # scraped from official UK Health CommGovion report, by date reporting end = 11-02-2020
...
death_observation_censoring <- as.numeric(as.Date("2020-01-29") - min(data$date)) # Assume no deaths detecte before 21st Jan in UK CDC numbers , (this kinda doesnt do too much)
deaths_x <- round(prop_deaths_NorthWest * sum(observed_deaths))
deaths_n <- sum(observed_deaths)
# Gov-covid19-bollettino-20200616:
Gov_observed_deaths <- 1188
Gov_observed_cases <-  1069706
# Input data
x <- c(0, n_age_bands,
