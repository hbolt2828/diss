...
observed_cases <- c(74164, 246567, 306591, 183342, 121813, 82320, 33457, 14565, 6887) #change for just england?
prop_NorthWest <- 398 / 1188
crude_cfr <- 1188 / 1069706
...
best_y <- min(data[data$location == "NorthWest", "nici"])
...
predicted <- bind_rows(lapply(1:nrow(m_thin), function(x, data2, m_thin, best_z){
  ...
  data2 %>% mutate(
    age60 = ifelse(age_groups %in% c("0-9", "10-19", "20-29", "30-39",
                                     "40-49", "50-59"), "U60", "O60"),
    cases_adjusted = cases * ifelse(location == "NorthWest", 1/best_y, 1/best_z) * nici,
